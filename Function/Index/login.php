...
class qq_qrlogin{
	...
	public function qqlogin(){
		...
		$sidurl='http://ptlogin2.qzone.com/ptqrlogin?u1=http%3A%2F%2Fm.qzone.com%2Finfocenter%3Fg_f%3D&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=0-0-'.time().'7886&js_ver=10034&js_type=1&login_sig='.$sig.'&pt_uistyle=32&aid=549000912&daid=147&';
		$ret = $this->get_curl($url,0,0,'qrsig='.$qrsig.'; ',1);
		$sidret = $this->get_curl($sidurl,0,0,'qrsig='.$qrsig.'; ',1);
		if(preg_match("/ptuiCB\('(.*?)'\);/", $ret, $arr) && preg_match("/ptuiCB\('(.*?)'\);/", $sidret, $sidarr)){
			$r=explode("','",str_replace("', '","','",$arr[1]));
			$sr=explode("','",str_replace("', '","','",$sidarr[1]));
			if($r[0]==0){
				preg_match('/Set-Cookie: ptnick_(.*?)=/',$ret,$uin);
				$uin=$uin[1];
				...
				if($data) {
					preg_match("/p_skey=(.*?);/", $data, $matchs);
					$pskey2 = $matchs[1];
				preg_match_all('/Set-Cookie: (.*);/iU',$data,$matchss);
				foreach ($matchss[1] as $val) {
					$cookies.=$val.'; ';
				}
				}
				if($pskey){
					exit('ptuiCB("0","'.$uin.'","@'.$skey[1].'","'.$pskey.'","'.$pskey2.'","'.$pookie.'","'.$cookies.'");');
				}else{
