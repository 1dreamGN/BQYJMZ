...
if($_SERVER['HTTP_REFERER']!="http://{$domain}/Function/Index/qiuqiu.php")exit("Error".$_SERVER['HTTP_REFERER']);
if ($_GET['type'] == "qiu") {
    $url = $_POST['url'];
    if (!$url) {
        exit('{"code":-1,"msg":"邀请链接不能为空"}');
    } else {
        $data = get_curl("http://auth.odoto.cc/qiuurl.php?urls=" . $url . "&siteurl=" . $domain);
		$data = str_replace("\r\n","",$data);
